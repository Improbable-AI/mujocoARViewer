...
def zip_scene_dir(scene_xml: Path) -> tuple[Path, str]:
    ...
    # Include the parent directory as the top-level folder in the ZIP
    # so the server sees the same structure after extraction.
    xml_relpath = str(Path(base_dir.name) / scene_xml.name)
    ...
    with zipfile.ZipFile(tmp_path, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for p in base_dir.rglob("*"):
            if p.is_file():
                arcname = Path(base_dir.name) / p.relative_to(base_dir)
                z.write(p, arcname)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument("--server", default="http://improbable382.csail.mit.edu:8080", help="Base URL of the converter server")
